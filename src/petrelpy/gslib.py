...
def get_midpoint_cell_columns(df, dir_out):
    ...
    df_i_or_j = df[
        (df["i_index"].isin(df_midpoints["i_index"]))
...
def agg_ijz(
    df_ijmatched, df_midpoints=None, zdmax=10, agg_arg="mean", uwi_col="UWI-Index"
):
    ...
    for _, (i, j, z, uwi) in df_midpoints[
...
def get_facies_histograms(
    ...
    ooip_splits = pd.DataFrame(
        columns=pd.MultiIndex.from_product(
            [zones.values, facies.values, properties],
